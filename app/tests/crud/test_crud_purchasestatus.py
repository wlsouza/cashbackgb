...
@pytest.mark.asyncio
async def test_if_get_by_id_return_correct_purchase_status(db: AsyncSession) -> None:
    ps_dict = random_purchase_status_dict()
...
@pytest.mark.asyncio
async def test_if_get_by_name_return_correct_purchase_status(db: AsyncSession) -> None:
    created_purchase_status = await crud.purchase_status.create(
